function _whois(args){
	$('#console').disableInput();
	_arg = fixparameter(args);
	_arg = _arg.split(" ");
	if(Object.size(_arg)!=fulllist["whois"]["ARGS"]||args=="") //Check syntax
	{
		_ret("The correct syntax is: '<b class='hlight'>whois</b> domain'.",false);
		$('#console').enableInput();
	}else{
		domain = args;
		$.ajax({
			type: "GET",
			timeout: 120000,
			url: "http://network-tools.com/default.asp?prog=whois&host="+domain,
			error: function(){
			...
			success: function(data){
				if(data["responseText"]==null){
					...
					res =  $(data["responseText"]).find("#wrap2").html();
